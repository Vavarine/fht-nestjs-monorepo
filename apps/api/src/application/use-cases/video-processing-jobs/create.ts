...
export class CreateVideoProcessingJob {
    ...
    async execute(request: CreateVideoProcessingJobRequest): Promise<CreateVideoProcessingJobResponse> {
        ...
        const createdVideoProcessingJob = await this.videoProcessingJobRepository.create(videoProcessingJob);

        await this.VideoProcessingPublisherJob.publish(createdVideoProcessingJob.id, createdVideoProcessingJob.videoFile);
