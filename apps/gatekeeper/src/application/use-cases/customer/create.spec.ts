...
describe('CreateCustomer', () => {
  ...
  beforeEach(() => {
    ...
    createCustomer = new CreateCustomer(customerIdentityRepository, customerDataRepository);
  });
  ...
  it('should create a customer successfully', async () => {
    ...
    const savedCustomer = await customerDataRepository.findById(response.customerId);
  ...
  it('should be able to find created customer by CPF', async () => {
    ...
    const foundCustomer = await customerDataRepository.findByCpf(request.cpf!);
  ...
  it('should create customers with different IDs', async () => {
    ...
    const customer1 = await customerDataRepository.findById(response1.customerId);
    const customer2 = await customerDataRepository.findById(response2.customerId);
  ...
  it('should throw CustomerAlreadyExistsError when customer with same CPF already exists', async () => {
    ...
    await expect(createCustomer.execute(newCustomer))
      .rejects
      .toThrow(CustomerAlreadyExistsError);
  });
});